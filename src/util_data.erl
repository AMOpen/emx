...
-export([close_handle/1, convert/2, delete_data/2,
	 foldl/3, get_data/2, get_handle/3, get_size/1,
	 get_type/1, put_data/2]).

put_data({ets, TableId}, Value) ->
    ets:insert(TableId, Value);
put_data({dets, TableId}, Value) ->
    dets:insert(TableId, Value);
put_data(remote, Value) -> nothing.
...
get_handle(dets, TableName, Options) ->
    {ok, Handle} =
	dets:open_file(get_dets_filename(TableName), Options),
    {dets, Handle}.

close_handle({ets, TableId}) -> ets:delete(TableId);
close_handle({dets, TableId}) -> dets:close(TableId);
close_handle(remote) -> nothing.
...
get_size({dets, TableId}) ->
    {dets:info(TableId, no_keys), 0,
     dets:info(TableId, memory)};
get_size(_) -> {0, 0, 0}.

get_type({Type, _}) -> Type;
get_type(undefined) -> system;
get_type(_) -> remote.
