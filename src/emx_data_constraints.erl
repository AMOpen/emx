...
-export([run_constraint/3, run_constraints/2]).
...
run_constraints2(_TableConfig, [], StartTime) -> ok;
    ...
    case should_bail(StartTime) of
	  ...
	  case TableConfig#emxstoreconfig.location of
		...
		case lists:any(fun (Node) -> Node == node() end,
			       TableConfig#emxstoreconfig.location)
		    of
		  true -> run_constraint(TableConfig, H, StartTime);
		  false -> null
		end
	  end
    end,
...
run_constraint(TableConfig, {records, MaxCount},
    ...
    {RecordCount, _, _} =
	util_data:get_size(TableConfig#emxstoreconfig.tableid),
    case RecordCount > MaxCount of
	  ...
	  lists:takewhile(fun (Record) ->
				  ?LOG(debug,
				       "Removing ~p to keep number of records "
				       "in limit",
				       [Record#emxcontent.displayname]),
				  util_data:delete_data(TableConfig#emxstoreconfig.tableid,
							Record#emxcontent.displayname),
				  util_zip:cleanup_record(Record),
				  {NewRecordCount, _, _} =
				      util_data:get_size(TableConfig#emxstoreconfig.tableid),
				  NewRecordCount > MaxCount andalso
				    should_not_bail(StartTime)
			  end,
			  SortedList),
...
run_constraint(TableConfig, {age, MaxAge}, StartTime) ->
    ...
    util_data:foldl(fun (Record, AccIn) ->
			    WriteTime =
				calendar:datetime_to_gregorian_seconds(Record#emxcontent.writetime),
			    case WriteTime < TestTime of
			      true ->
				  ?LOG(debug, "Removing ~p as it is old",
				       [Record#emxcontent.displayname]),
				  util_data:delete_data(TableConfig#emxstoreconfig.tableid,
							Record#emxcontent.displayname),
				  util_zip:cleanup_record(Record);
		    ...
		    [], TableConfig#emxstoreconfig.tableid);
...
run_constraint(TableConfig, {size, MaxSize},
    ...
    case Memory > MaxSize of
	  ...
	  SortedList = get_all_sorted(TableConfig),
	  lists:takewhile(fun (Record) ->
				  ?LOG(debug, "Removing ~p to free up memory",
				       [Record#emxcontent.displayname]),
				  util_data:delete_data(TableConfig#emxstoreconfig.tableid,
							Record#emxcontent.displayname),
				  util_zip:cleanup_record(Record),
...
run_constraint(TableConfig, {archive, ArchiveAge},
    ...
    util_data:foldl(fun (Record, AccIn) ->
				...
				calendar:datetime_to_gregorian_seconds(Record#emxcontent.writetime),
			    IsArchived =
				getArchiveStatus(Record#emxcontent.content),
			    case {WriteTime < TestTime, IsArchived,
				  should_not_bail(StartTime)}
				of
			      {true, false, true} ->
				  ...
				  case NewRecord of
				    Record -> nothing;
				    _ ->
					?LOG(debug, "Archived ~p as it is old",
					     [Record#emxcontent.displayname]),
					util_data:put_data(TableConfig#emxstoreconfig.tableid,
							   NewRecord)
				  end;
			      _ -> ok
			    end,
...
getArchiveStatus({archived, _FileName}) -> true;
getArchiveStatus({compressed, _Data}) -> false;
getArchiveStatus(_Data) -> false.

%% Get data from a table and sort it by age

get_all_sorted(TableConfig) ->
    UnsortedList = util_data:foldl(fun (Record, AccIn) ->
					   AccIn ++ [Record]
				   end,
				   [], TableConfig#emxstoreconfig.tableid),
    lists:sort(fun (Elem1, Elem2) ->
		       First =
			   calendar:datetime_to_gregorian_seconds(Elem1#emxcontent.writetime),
		       Second =
			   calendar:datetime_to_gregorian_seconds(Elem2#emxcontent.writetime),
		       Second > First
	       end,
	       UnsortedList).
