...
-export([get_data/1, get_datakeys/1, get_datakeys/2,
	 get_prefixes/0, housekeep/0, put_data/2, put_data/3,
	 put_file/3]).
...
init(_) -> process_flag(trap_exit, true), {ok, []}.
...
put_data(Key, Content, Encoding) ->
    ...
    gen_server:call(?GD2,
		    {putData, normaliseKey(Key), Content, Encoding},
		    infinity).

get_data(Key) ->
    gen_server:call(?GD2, {getData, normaliseKey(Key)},
		    30000).
...
get_datakeys(Prefix, EpochNumber) ->
    gen_server:call(?GD2,
		    {getDataKeys, Prefix, EpochNumber}, infinity).

get_datakeys(Prefix) -> get_datakeys(Prefix, 0).
...
denormaliseTable(Table) when is_atom(Table) ->
    One = string:join(string:tokens(atom_to_list(Table),
				    "_"),
		      "/"),
    string:join(string:tokens(One, "~"), " ");
denormaliseTable(Table) -> Table.

%% API HANDLING FUNCTIONS

%% TODO: Add writeUser to this call
handle_call({putData, Key, Content, Encoding}, _From,
	    N) ->
    ...
    Data = #emxcontent{displayname = Key,
		       writetime = calendar:local_time(), writeuser = anon,
...
handle_call({getPrefixes}, _From, N) ->
    util_check:check_message_queue(),
    Prefixes =
	[denormaliseTable(Record#emxstoreconfig.typename)
	 || Record <- emx_data:get_tables()],
    {reply, Prefixes, N};
...
handle_call({getDataKeys, Prefix, EpochNumber}, _From,
	    N) ->
    util_check:check_message_queue(),
    {datainfo, {MaxEpoch, Keys}} =
	emx_data:get_datakeys(getTableId(Prefix), EpochNumber),
    {reply, {datainfo, {MaxEpoch, Keys}}, N};
handle_call({getData, Key}, _From, N) ->
    util_check:check_message_queue(),
    {datainfo, Res} = emx_data:get_data(getTableId(Key),
					Key),
    case Res of
      [] -> {reply, nodata, N};
...
handle_call({housekeep}, _From, N) ->
    ...
    {message_queue_len, Len} = erlang:process_info(self(),
						   message_queue_len),
    case Len > 100 of
      true -> ?LOG(debug, "Too busy to run housekeeping", []);
      false ->
