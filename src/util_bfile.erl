...
-export([delete_content/1, get_table_keys/1,
	 load_content/1, save_content/2]).
...
get_table_keys(TablePrefix) ->
    ...
    %%DataDir = "c:\\clouddata\\",
    PrefixAsStringFolder =
	string:join(string:tokens(atom_to_list(TablePrefix),
				  "_"),
		    "\\"),
    Prefix =
	string:join(string:tokens(atom_to_list(TablePrefix),
				  "_"),
		    "/"),
    StartDirectory = atom_to_list(DataDir) ++
		       "/" ++ PrefixAsStringFolder,
    ...
    case Res of
	  ...
	  {Files, Folders} = lists:foldl(fun (File, {Fi, Fo}) ->
						 FullFile = Folder ++
							      "\\" ++ File,
						 {ok, FileInfo} =
						     file:read_file_info(list_to_atom(FullFile)),
					 ...
					 {[], []}, FilesHere),
	  RootRet = [util_string:format("~s/~s", [Prefix, File])
		     || File <- Files],
...
delete_content(Key) when is_list(Key) ->
    RealFile = get_real_file(Key), file:delete(RealFile).
