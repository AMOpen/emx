...
processBalanceNode(ConfigHandle, TableInfo, false,
		   Count)
    when Count == 0 ->
    util_data:delete_data(ConfigHandle,
...
processBalanceNode(ConfigHandle, TableInfo, false,
    ...
    ?LOG(debug, "Asking for getkeys from node ~p",
	 [TestNode]),
    DataList = rpc:call(TestNode, emx_data, get_datakeys,
			[TableInfo#emxstoreconfig.typename, 0]),
    case DataList of
      {badrpc, _} ->
	  ?LOG(debug, "Could not obtain information from ~p",
	       [TestNode]);
      {datainfo, {_MaxEpoch, []}} -> nothing;
      {datainfo, {MaxEpoch, List}} ->
	  NewTableId =
	      util_data:get_handle(TableInfo#emxstoreconfig.storagetype,
				   TableInfo#emxstoreconfig.typename,
				   TableInfo#emxstoreconfig.storageoptions),
	  NodeList = TableInfo#emxstoreconfig.location ++
		       [node()],
	  UpdatedTableInfo = TableInfo#emxstoreconfig{location =
							  NodeList,
						      tableid = NewTableId},
	  lists:foreach(fun (Record) ->
				?LOG(debug, "Copying ~p with ~p records",
	  ...
	  emx_data_constraints:run_constraints(UpdatedTableInfo,
					       UpdatedTableInfo#emxstoreconfig.capacityconstraints),
	  ?LOG(debug, "New epoch is ~p", [MaxEpoch]),
	  util_data:put_data(ConfigHandle,
			     UpdatedTableInfo#emxstoreconfig{epoch = MaxEpoch}),
	  %% Now inform all of the other nodes that have this table that we have it too
	  {ok, Nodes} = application:get_env(nodes),
	  MyNode = node(),
	  lists:foreach(fun (Node) ->
				?LOG(debug, "Informing ~p of the new node",
				     [Node]),
				case Node of
				  MyNode -> nothing;
				  _ ->
				      rpc:call(Node, emx_data,
					       update_table_info,
					       [TableInfo#emxstoreconfig.typename,
						nodeup, MyNode])
				end
...
processBalanceNode(_ConfigHandle, TableInfo, true,
		   Count)
    when Count > 2 ->
    ?LOG(debug, "Should remove balance ~p",
	 [TableInfo#emxstoreconfig.typename]);
processBalanceNode(ConfigHandle, TableInfo, true, _) ->
    ...
    case {Size > 50000, Type} of
	  ...
	  NewTableInfo = TableInfo#emxstoreconfig{tableid =
						      util_data:convert(TableInfo#emxstoreconfig.tableid,
									TableInfo#emxstoreconfig.typename)},
	  util_data:put_data(ConfigHandle, NewTableInfo);
      {false, dets} ->
	  ?LOG(debug,
	       "Converting disk table to memory table "
	       "for ~p",
	       [TableInfo#emxstoreconfig.typename]),
	  NewTableInfo = TableInfo#emxstoreconfig{tableid =
						      util_data:convert(TableInfo#emxstoreconfig.tableid,
