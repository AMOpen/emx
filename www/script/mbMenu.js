	...
	buildMenu : function (options){
		return this.each (function ()
		{
			...
			$(root).each(function(){
				...
				if (!openOnClick){
					$(thisMenu).bind("mouseout",function(){
						//$(thisMenu.rootMenu).remove();
	...
	buildContextualMenu :  function (options){
		return this.each (function ()
		{
			...
			$(cMenuEls).each(function(){
				...
				//$(this).bind("contextmenu",function(){event.stopPropagation(); event.cancelBubble=true})
				$(this).bind("contextmenu","mousedown",function(event){
					if (event.which==3){
